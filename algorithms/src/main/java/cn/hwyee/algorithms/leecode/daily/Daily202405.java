...
//        System.out.println(solution121.minDays(429));
        Solution_29_1 solution291 = new Solution_29_1();
        System.out.println(solution291.maximumLength("biaei"));
    }
...
//                        max = Math.max(max, count);
            ...
            for (Map.Entry<Character, List<Integer>> characterListEntry : map.entrySet()) {
                int size = characterListEntry.getValue().size();
                int tempmax = characterListEntry.getValue().stream().max(Integer::compareTo).get();
                int min = characterListEntry.getValue().stream().min(Integer::compareTo).get();
                if (size>=3){
                    int maxcount = 0;
                    int secondmax = 0;
                    for (Integer integer : characterListEntry.getValue()) {
                        if(maxcount==3){
                            break;
                        }
                        if(integer == tempmax){
                            maxcount++;
                        }else if(tempmax - 1 == integer){
                            secondmax = integer;
                        }
                    }
                    if(maxcount==3){
                        max = Math.max(max,tempmax);
                    }else if (maxcount == 2 || secondmax !=0){
        ...
        public int maximumLength_LS(String S) {
     ...
     * @version 1.0 
     * @return 
     * @date 2024/5/31 13:10
     */
    class Solution_31_1 {
        public int[] findMissingAndRepeatedValues(int[][] grid) {
            int[] ans = new int[2];
            int[] all = new int[grid.length*grid.length+1];
            int gsum = 0;
            int sum = 0;
            int c = 0;
            for(int i = 0;i<grid.length;i++){
